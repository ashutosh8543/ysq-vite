    ...
    public function storeLanguage($input)
    {
        try {
            ...
            if($input['lang_image']){  
                ...
                $extension = explode('/', mime_content_type($input['lang_image']))[1];               


                $image =$input['lang_image'];  // your base64 encoded
                $image = str_replace('data:image/'.$extension.';base64,', '', $image);
                $image = str_replace(' ', '+', $image);
                $imageName = rand().'.'.$extension;
                $destinationPath = 'language';    
                $input['image']=url('public/language'.'/'. $imageName);
