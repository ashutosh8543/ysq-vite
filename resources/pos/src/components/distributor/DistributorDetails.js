...
const DistributorDetails = (props) => {
    ...
    const editLink=allConfigData?.permissions && allConfigData?.permissions.includes(Permissions.EDIT_DISTRIBUTOR)?"/app/distributor/edit/"+id:"";
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle title={getFormattedMessage('distributor-details.title')} to='/app/distributor' editLink={editLink}/>
                                    ...
                                    <div
                                        ...
                                        <div className='ms-0 ms-md-10 mt-5 mt-sm-0'>
                                            ...
                                            <span className='text-warning'>
                                                <strong>
                                                {user && user.attributes?.unique_code}</strong></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <Card>
                        ...
                        <Card.Body className='pt-0'>
                            <Table responsive>
                                <tbody>
                                <tr>
                                    <td className='py-4'>{getFormattedMessage('globally.input.country.label')}</td>
                                    <td className='py-4'>{user && user.attributes?.countryDetails?.name}</td>
                                </tr>
                                <tr>
                                    <td className='py-4'>Region</td>
                                    <td className='py-4'>{user && user.attributes?.regionDetails?.name}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td className='py-4'>{moment(user && user.attributes.created_at).format('DD-MM-yyyy')}</td>
                                </tr>
                                </tbody>
                            </Table>
                        </Card.Body>
